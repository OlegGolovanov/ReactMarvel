...
class CharInfo extends Component {
    ...
    render(){
        const newState = this.state.chars.filter(item=> {
            return item.id == this.props.id
        });
        const charInfo = newState.length == 0 ? <Skeleton/> : <Char char = {newState}/>
        return (
            <>
                {charInfo}
            </>
...
const Char = ({char}) => {    
    const {name, img, homepage, wiki, description, id} = char[0];
    console.log(name);   
    return(
        <div className="char__info">
            <div className="char__basics">
